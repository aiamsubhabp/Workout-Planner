...
if __name__ == "__main__":
  with app.app_context():
    ...
    u1_e4 = UserExercise(sets = 5, reps = 5, user = u1, exercise = e4)
    u1_e5 = UserExercise(sets = 5, reps = 5, user = u1, exercise = e5)
    u1_e6 = UserExercise(sets = 5, reps = 5, user = u1, exercise = e6)

    db.session.add_all([u1_e4, u1_e5, u1_e6])

    u2_e5 = UserExercise(sets = 3, reps = 10, user = u2, exercise = e5)
    u2_e1 = UserExercise(sets = 5, reps = 20, user = u2, exercise = e1)
    u2_e9 = UserExercise(sets = 5, reps = 10, user = u2, exercise = e9)

    db.session.add_all([u2_e1, u2_e5, u2_e9])

    u3_e2 = UserExercise(sets = 3, reps = 10, user = u3, exercise = e2)
    u3_e3 = UserExercise(sets = 3, reps = 10, user = u3, exercise = e3)
    u3_e8 = UserExercise(sets = 3, reps = 10, notes = 'need big arms', user = u3, exercise = e8)
    u3_e9 = UserExercise(sets = 3, reps = 10, user = u3, exercise = e9)
